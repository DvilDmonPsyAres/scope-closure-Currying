...
const smoothieMachine = (...ingredients) => {
  ...
  if(ingredients.length === 1) {
    str = str + ingredients.join("")
  }
  else if(ingredients.length > 1) {
    str = str + ingredients.join(" and ")
  }
  return (...newIngredients) => {
    if(str.length < 28 && newIngredients.length === 1) {
      str = str + newIngredients.join("");
    } else if (str.length < 28 && newIngredients.length > 1){
      str = str + newIngredients.join(" and ");
    } else if (newIngredients.length === 1) {
      str = str + " and " + newIngredients.join("");
    } else if (newIngredients.length > 1) {
      str = str + " and " + newIngredients.join(" and ");
    }
    return str;
  }
};
